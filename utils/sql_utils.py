...
def mssql_engine(
    ...
    engine = db.create_engine(
        f"mssql+pyodbc://{config['username']}:{config['password']}@{config['server']}/DGS_FasterWeb?driver=SQL+Server"
    )
    return engine


def get_data_from_archibus(engine):
    query = "SELECT * from DGS_Archibus.afm.wrhwr;"
    faster_df = pd.read_sql(query, engine)
    return faster_df
